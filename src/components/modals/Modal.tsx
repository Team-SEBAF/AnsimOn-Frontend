...
function Root({ open, onOpenChange, className, children }: ModalRootProps) {
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        className={cn(
          // 컨테이너 기본 스타일 (카드 외형/여백/레이아웃)
          'flex min-h-50 w-[320px] flex-col justify-between gap-6 rounded-xl border border-neutral-100 bg-white px-7 py-6 shadow-2xl',
          className,
        )}
      >
        {children}
...
function Header({ title, subTitle, className }: ModalHeaderProps) {
  return (
    <DialogHeader className={className}>
      <DialogTitle className="typo-heading-2 mb-1 text-neutral-900">{title}</DialogTitle>
